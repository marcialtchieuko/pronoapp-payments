...
// Pays utilisant le XAF (zone CEMAC)
const XAF_COUNTRIES = ["CM", "GA", "CG", "TD", "CF", "GQ"];
// Pays utilisant le XOF (zone UEMOA)
const XOF_COUNTRIES = ["SN", "CI", "ML", "BF", "NE", "TG", "BJ", "GW"];
// Pays d'Europe utilisant l'euro (liste large, pas exhaustive)
const EUROPE_COUNTRIES = [
  "FR", "DE", "ES", "IT", "PT", "BE", "NL", "LU", "IE", "AT",
  "FI", "GR", "SK", "SI", "EE", "LV", "LT", "CY", "MT", "HR",
  "GB", "CH", "SE", "DK", "NO", "PL", "CZ", "RO", "HU", "BG",
];

function detectRegionFromCountry(countryCode) {
  if (!countryCode) return null;
  if (XAF_COUNTRIES.includes(countryCode)) return "afrique";
  if (XOF_COUNTRIES.includes(countryCode)) return "afrique_ouest";
  if (EUROPE_COUNTRIES.includes(countryCode)) return "europe";
  return null;
}
...
async function handleCreatePayment(request, env) {
  try {
    ...
    const detectedCountry = request.cf?.country || "";
    console.log("create-payment body reçu:", JSON.stringify(body), "pays détecté:", detectedCountry);

    const autoRegion = detectRegionFromCountry(detectedCountry);
    const regionKey = autoRegion || (PLANS[region] ? region : "afrique");
    const planInfo = PLANS[regionKey][plan];
    console.log("regionKey retenue:", regionKey, "(auto:", autoRegion, ") planInfo:", JSON.stringify(planInfo));
